...
export default function PersonRegistration() {
  ...
  return (
    ...
    <div style={{ maxWidth: 600, margin: '40px auto', padding: 24, background: '#fff', borderRadius: 16, boxShadow: '0 8px 32px rgba(0,0,0,0.08)' }}>
      ...
      <div style={{ marginBottom: 8, minHeight: 380, background: '#0b1120', borderRadius: 12, overflow: 'hidden', position: 'relative' }}>
        {cameraActive ? (
          <RegistrationCamera onFaceScan={handleFaceScan} disabled={showModal || countdown > 0} />
        ) : (
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#e5e7eb', padding: 16 }}>
            <div style={{ marginBottom: 12, fontSize: 14 }}>Camera is currently off.</div>
            <button
              type="button"
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.5)', zIndex: 1000 }}>
          ...
          >
            ...
            <PersonDetails scanPayload={pendingScan} onComplete={closeModal} hidePersonTable />
          </div>
        </div>
      )}
    </div>
