...
export default function ReleasedHistoryPayroll() {
  ...
  const handleExportExcel = () => {
    ...
    const exportData = sortedPayrolls.map(row => ({
      ...
      'Late Deduction': modalData.payroll && selected && selected.id === row.id
              ...
              {sortedPayrollsFinal.length === 0 ? (
                ...
                sortedPayrollsFinal.map((p, idx) => (
                  <tr key={p.id} style={{ ...styles.tr, backgroundColor: idx % 2 === 0 ? '#f9fafb' : '#fff' }}>
                    ...
                    {(() => {
                      const isSelected = modalData.payroll && selected && selected.id === p.id;
                      const fromModal = modalData.payroll || {};

                      const dailyRate = isSelected
                        ? (fromModal.dailyRate ?? fromModal.daily_rate ?? p.daily_rate)
                        : p.daily_rate;
                      const latePenalty = isSelected
                        ? (fromModal.latePenalty ?? fromModal.late_penalty ?? p.late_penalty)
                        : p.late_penalty;
                      ...
                      const lateDeduction = isSelected
                        ? (fromModal.totalLateDeduction ?? p.total_late_deduction ?? p.late_deduction)
                        : (p.total_late_deduction ?? p.late_deduction);
                      const net = isSelected
                        ? (fromModal.net ?? p.net)
                        : p.net;

                      return (
                        <>
                          <td style={styles.td}>₱{dailyRate != null ? Number(dailyRate).toFixed(2) : '-'}</td>
                          <td style={styles.td}>₱{latePenalty != null ? Number(latePenalty).toFixed(2) : '-'}</td>
                          <td style={styles.td}>{daysPresent != null ? daysPresent : '-'}</td>
                          <td style={styles.td}>{lateCount != null ? lateCount : '-'}</td>
                          <td style={styles.td}>₱{gross != null ? Number(gross).toLocaleString() : '-'}</td>
                          <td style={styles.td}>₱{lateDeduction != null ? Number(lateDeduction).toLocaleString() : '-'}</td>
                          <td style={styles.td}>₱{net != null ? Number(net).toLocaleString() : '-'}</td>
                        </>
                      );
                    })()}
                    <td style={styles.td}>
